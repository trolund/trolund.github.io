...
export default function AboutHero({ content }: AboutHeroProps) {
  return (
    <section className="border-border-color text-content-text shadow-custom bg-(--bg) relative overflow-hidden rounded-[36px] border">
      ...
      <div className="relative grid gap-8 p-5 md:p-8 lg:grid-cols-[320px_1fr] lg:gap-12 lg:p-10">
        <div className="flex flex-col items-center gap-6 text-center lg:items-start lg:text-left">
          ...
          <dl className="text-content-text grid w-full grid-cols-2 gap-4 text-left">
            {highlights.map((item) => (
              <div
                key={item.label}
                className="border-border-color bg-white/72 hover:bg-white/86 min-w-0 rounded-[24px] border p-4 text-sm shadow-[0_16px_36px_rgba(15,23,42,0.06)] backdrop-blur-xl transition-colors dark:bg-white/5 dark:hover:bg-white/10"
              >
        ...
        <div className="border-border-color bg-white/74 rounded-[30px] border p-6 shadow-[0_24px_70px_rgba(15,23,42,0.08)] backdrop-blur-xl dark:bg-white/5">
