...
export default function AboutHero({ content }: AboutHeroProps) {
  return (
    <section className="border-border-color text-content-text shadow-custom relative overflow-hidden rounded-[36px] border bg-(--bg)">
      ...
      <div className="relative grid gap-8 p-5 md:p-8 lg:grid-cols-[320px_1fr] lg:gap-12 lg:p-10">
        <div className="flex flex-col items-center gap-6 text-center lg:items-start lg:text-left">
          ...
          <div className="space-y-3">
            <p className="text-content-text/55 text-xs font-semibold tracking-[0.35em] uppercase">
              Software Engineer
            </p>
            <h2 className="text-4xl leading-tight font-semibold tracking-tight text-balance md:text-[2.8rem]">
              Troels Elsvad Lund
            </h2>
            <p className="text-content-text/72 text-sm leading-7 text-pretty md:text-base">
          ...
          <dl className="text-content-text grid w-full grid-cols-2 gap-4 text-left">
            {highlights.map((item) => (
              <div
                key={item.label}
                className="border-border-color min-w-0 rounded-[24px] border bg-white/72 p-4 text-sm shadow-[0_16px_36px_rgba(15,23,42,0.06)] backdrop-blur-xl transition-colors hover:bg-white/86 dark:bg-white/5 dark:hover:bg-white/10"
              >
                <dt className="text-content-text/55 text-[0.65rem] font-semibold tracking-[0.3em] uppercase">
                  {item.label}
                </dt>
                <dd className="text-[clamp(1rem,2.1vw,1.3rem)] leading-tight font-semibold text-balance">
                  {item.value}
                </dd>
        ...
        <div className="border-border-color rounded-[30px] border bg-white/74 p-6 shadow-[0_24px_70px_rgba(15,23,42,0.08)] backdrop-blur-xl dark:bg-white/5">
