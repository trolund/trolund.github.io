...
export default function CardScrollTailwind() {
    ...
    return (
        <div
            ref={containerRef}
            className="flex overflow-x-scroll scroll-snap-x mandatory px-10 py-20 gap-[40px] no-scrollbar"
        ...
        >
            {cardsData.map((card, index) => {
                ...
                return (
                    <div
                        key={card.id}
                        className={`${card.color} rounded-2xl flex-shrink-0 w-[280px] h-[400px] flex items-center justify-center text-white font-bold text-xl select-none`}
                        style={{
